...
class ThreadedCamera:
    def __init__(self, api_key, host, port):
        self.active = True
        self.results = []
        self.capture = cv2.VideoCapture("rtsp://admin:dev@2022@192.168.1.19/cam/realmonitor?channel=3subtype=1")
        # self.capture = cv2.VideoCapture("rtsp://admin:Admin@123@192.168.1.111/cam/realmonitor?channel=1subtype=1")
                        ...
                        if subjects:
                            ...
                            if similarity*100 > 95:
                                ...
                                if count==check:
        		                    ...
        		                    VALUES (?, ?, ?, ?)""", (2,subject,probability,datetime.datetime.now()))
                                    cnxn.commit()
                                    check=check+5
                                count=count+1
